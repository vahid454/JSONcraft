    ...
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden",
      ...
      <header style={{ flexShrink:0, height:46, borderBottom:`1px solid ${T.border}`,
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          {parsed && (
            <span style={{ fontSize:11, color: dotColor, padding:"2px 8px",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:5 }}>
          ...
          <button onClick={() => setShowDiff(true)} title="Diff two JSONs" style={toolBtn(showDiff)}>
            ± Diff
          </button>
          <button onClick={() => setShowJWT(true)} title="JWT Encode/Decode" style={toolBtn(showJWT)}>
            🔐 JWT
          </button>
          <button onClick={() => setShowTools(true)} title="Dev Tools: RegEx, Hash, Encode" style={toolBtn(showTools)}>
            🛠 Tools
          </button>
          <div style={{ width:1, height:18, background: T.border }} />
          <button onClick={() => setShowContact(true)}    title="Contact"    style={{ ...hdrBtn(false), padding:"5px 9px", fontSize:15 }}>✉</button>
          <button onClick={() => setDark(!dark)}          title="Toggle theme" style={{ ...hdrBtn(false), padding:"5px 9px" }}>{dark ? "☀" : "☾"}</button>
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden", minHeight:0 }}>
        ...
        <div
          style={{ ...col, width:"50%", borderRight:`1px solid ${T.border}`,
            outline: draggingOver ? "2px dashed #10b981" : "none", outlineOffset:-2 }}
          ...
          <div style={{ flexShrink:0, padding:"5px 16px", borderBottom:`1px solid ${T.border}`,
            ...
            <span style={{ fontSize:10, color: T.mute2, opacity:0.6 }}>— drag & drop a file</span>
            {draggingOver && <span style={{ fontSize:10, color:"#10b981" }}>📂 Drop to load</span>}
            {input && (
        ...
        <div style={{ ...col, flex:1 }}>
          ...
          <div style={{ flexShrink:0, borderBottom:`1px solid ${T.border}`,
            background: T.bg, display:"flex", padding:"0 12px" }}>
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
                padding:"9px 12px", fontSize:11, letterSpacing:"0.06em", textTransform:"uppercase",
          ...
          <div style={{ flex:1, minHeight:0, overflowY:"auto", overflowX:"hidden",
            ...
            {!input && !error && activeTab !== "convert" && (
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
                ...
                <div style={{ fontSize:48, fontWeight:700, color: T.mute }}>{"{}"}</div>
                <div style={{ fontSize:13, color: T.mute }}>Paste JSON · XML · CSV · YAML</div>
                <div style={{ fontSize:11, color: T.mute }}>or drag & drop a file · or load from URL</div>
              </div>
            )}
      ...
      {showDiff       && <DiffPanel       data={parsed} dark={dark} onClose={() => setShowDiff(false)} />}
      {showJWT        && <JWTModal        dark={dark} onClose={() => setShowJWT(false)} />}
      {showTools      && <ToolsModal      dark={dark} onClose={() => setShowTools(false)} initialData={parsed} />}
    </div>
  );
}
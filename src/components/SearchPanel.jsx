...
function ValueChip({ val, dark }) {
  ...
  if (val === null)             return <span style={{ color: colors.null,    fontSize: 12 }}>null</span>;
  if (typeof val === "boolean") return <span style={{ color: colors.boolean, fontSize: 12 }}>{String(val)}</span>;
  if (typeof val === "number")  return <span style={{ color: colors.number,  fontSize: 12 }}>{val}</span>;
  return <span style={{ color: colors.string, fontSize: 12 }}>"{String(val)}"</span>;
}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10, height:"100%", minHeight:0 }}>
      <div style={{ position:"relative", flexShrink:0 }}>
        ...
        {query && (
          <button onClick={() => setQuery("")} style={{
            ...
            color: mute, fontSize:14, fontFamily:"inherit", padding:0,
          }}>✕</button>
        )}
      </div>

      <div style={{ fontSize:12, color: mute, flexShrink:0 }}>
      ...
      <div style={{ flex:1, overflowY:"auto", minHeight:0, display:"flex", flexDirection:"column", gap:1 }}>
        {results.map(({ path, value }, i) => (
          ...
          >
            <span style={{ fontSize:12, color: dark ? "#94a3b8" : "#475569",
            ...
            <button onClick={() => copyVal(value, i)} style={{
              fontSize:12, color: copied === i ? "#10b981" : mute,
              background:"transparent", border:"none", cursor:"pointer",
